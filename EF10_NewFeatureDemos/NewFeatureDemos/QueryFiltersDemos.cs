...
public class QueryFiltersDemos : IAsyncDemo
{
    ...
    private async Task ShowNamedQueryFilters()
    {
        ...
        Console.WriteLine("Categories with no filter disabled [not soft deleted && active]");
        var categories = await _db.Categories.ToListAsync();
        Console.WriteLine(ConsolePrinter.PrintBoxedList(categories, c => $"{c.Id}: {c.CategoryName} [IS Deleted: {c.IsDeleted}] - [Is Active {c.IsActive}]"));



        Console.WriteLine("Categories with both filters disabled [any state for soft delete or active]");
        ...
        Console.WriteLine("Categories with soft delete filter disabled [any soft-delete state, but only active]");
        ...
        Console.WriteLine("Categories with active filter disabled [any active state, but only non-soft-deleted]");
